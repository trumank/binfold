...
fn print_disassembly_with_edges(raw_bytes: &[u8], base: u64) {
    ...
    // Print disassembly with edge information
    let mut formatter = iced_x86::NasmFormatter::new();
    let mut output = String::new();
    ...
    for (&addr, (instruction, _)) in &instructions {
        ...
        println!("0x{:08x}  | {:3} | {:3} | {}", addr, in_edges, out_edges, output);
...
mod test {
    ...
    fn test_binary_ninja_blocks() {
        ...
        for &(start, end) in &expected_blocks {
            ...
            println!("0x{:x} - 0x{:x} | Expected: 0x{:x} - 0x{:x} | Match: {}", 
                start, our_end.unwrap_or(&0), start, end, matches);
        }
    ...
    fn test_binary_ninja_blocks_old() {
        ...
        let base = 0x1400015ec;
        let blocks = identify_basic_blocks(&function_bytes, base);
        ...
        for &(start, end) in &expected_blocks {
            let our_end = blocks.get(&start);
            println!("0x{:x} - 0x{:x} | BN: 0x{:x} - 0x{:x} | Match: {}", 
                start, our_end.unwrap_or(&0), start, end, 
                our_end == Some(&end));
        }
